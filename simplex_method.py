...
def calculate_min_ratio(A, b, ratio_col):
    ...
    for row in range(len(A)):
        ...
        if 0 < ratio < min_ratio:
            ...
            min_ratio_row = row
    if min_ratio_row == -1:
        raise ValueError("Разрешающий элемент не найден.")
    ...
    if validate_simplex_input(c, A, b):
        ...
        while (max(c) > 0) or (min(b) < 0):
            ...
            if resolving_element == ["inf"]:
                ...
                return 1
            print("Найден разрешающий элемент:", resolving_element)
        ...
        print("Проверка ввода: не пройдена")
        return 1

    if minimize:
        print("Функция минимизирована.")
        return f
    print("Функция максимизирована.")
    return f * -1
